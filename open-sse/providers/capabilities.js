...
// Fork addition: feeds the fork-only dashboard capability-override feature
// (see docs/FORK.md). This is a deliberate, documented exception to the
// "open-sse/ is never modified" rule — relocating it is not viable because
// getCapabilitiesForModel has ~25 call sites (9 internal to open-sse/, e.g.
// handlers/chatCore.js) that resolve it at module-load time with no injection
// point from src/. Upstream's own setCatalogSource()/globalThis slot is the
// same in-module extension pattern. Keep both `Fork addition:` markers in sync.
...
export function getCapabilitiesForModel(provider, model) {
  ...
  // Fork addition: user capability override (from SQLite / dashboard).
  // This guard MUST stay above every early return in this function — including
  // the `commandcode`/`cmc` branch below and the provider/base table lookups —
  // or an override silently stops applying for the paths that return first
  // (upstream refactors this function often; a bypass shows up as a wrong
  // capability, not a merge conflict). Covered by tests/unit/capabilities-override.test.js.
