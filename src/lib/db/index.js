...
// Per-API-key access policies (fork addition)
export {
  getAllApiKeyPolicies, getApiKeyPolicy, getApiKeyPolicyRaw,
  updateApiKeyPolicy, deleteApiKeyPolicy, pruneApiKeyPolicy, initApiKeyPolicies,
} from "./repos/apiKeyPolicyRepo.js";
...
export async function exportDb() {
  ...
  // Fork addition: per-API-key access policies.
  for (const r of db.all(`SELECT key, value FROM kv WHERE scope = 'apiKeyPolicies'`)) out.apiKeyPolicies[r.key] = parseJson(r.value);
...
export async function importDb(payload) {
  ...
  db.transaction(() => {
    ...
    db.run(`DELETE FROM kv WHERE scope IN ('modelAliases', 'customModels', 'mitmAlias', 'pricing', 'apiKeyPolicies')`);
    ...
    // Fork addition: per-API-key access policies.
    for (const [keyId, policy] of Object.entries(payload.apiKeyPolicies || {})) {
      db.run(`INSERT OR REPLACE INTO kv(scope, key, value) VALUES('apiKeyPolicies', ?, ?)`, [keyId, stringifyJson(policy || {})]);
    }
  });
